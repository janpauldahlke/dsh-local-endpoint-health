...
function backendLabel(snapshot2) {
  if (snapshot2 === null) return null;
  switch (snapshot2.backend) {
    case "llama-cpp":
      return "llama.cpp";
    case "ollama":
      return snapshot2.backendVersion !== null && snapshot2.backendVersion !== "" ? `ollama ${snapshot2.backendVersion}` : "ollama";
    case "vllm":
      return "vllm";
    default:
      return null;
...
function bareStateWord(chip, snapshot2) {
  const engine = snapshot2 !== null ? backendWord(snapshot2.backend) : null;
  if (engine !== null) {
    const prefix = `${engine} \xB7 `;
...
function deriveChip(live, now) {
  ...
  const engine = backendWord(snapshot2.backend);
  ...
  if (snapshot2.state === "up-no-model") {
    return {
      state: "no-model",
  ...
  if (snapshot2.state === "up-loaded") {
    const loaded = snapshot2.ollama?.loaded ?? [];
...
function fmtExpiry(iso, now) {
  if (iso === null) return "\u2014";
  const t = Date.parse(iso);
  if (Number.isNaN(t)) return "\u2014";
  const d = t - now;
  if (d <= 0) {
    const s2 = Math.round(-d / 1e3);
    return s2 < 60 ? `expired ${s2} s ago` : `expired ${Math.floor(s2 / 60)} m ago`;
  }
  const s = Math.round(d / 1e3);
  if (s < 60) return `in ${s} s`;
  return `in ${Math.floor(s / 60)} m ${s % 60} s`;
}
function OllamaCard({ ollama, now }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
    CollapsibleCard,
    {
      ...
      meta: ollama.libraryCount !== null ? { text: `${ollama.libraryCount} in library`, title: "Local library size, from /api/tags" } : void 0,
      preview: [
        {
      ...
      children: [
        !ollama.fresh && ollama.error !== null && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("p", { style: { ...muted, overflowWrap: "anywhere" }, children: ollama.error }),
        count === 0 && ollama.fresh && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("p", { style: muted, children: "Nothing loaded. Ollama evicts idle models after their keep-alive (default 5 m); run a model to see it here." }),
        ollama.loaded.map((m) => /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { style: { display: "flex", flexDirection: "column", gap: 2, padding: "2px 0" }, children: [
          /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { style: { display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap" }, children: [
            /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { style: { fontWeight: 600 }, children: m.name }),
            /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { style: { ...muted, fontSize: 12 }, children: [m.family, m.parameterSize, m.quantization].filter((x) => x !== null).join(" \xB7 ") || "\u2014" })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { style: { display: "flex", gap: 16, fontSize: 12, fontVariantNumeric: "tabular-nums" }, children: [
            /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("span", { title: "Total model size, from /api/ps (size)", children: [
              "size ",
              fmtBytes(m.size)
            ] }),
            /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("span", { title: "Bytes resident in VRAM, from /api/ps (size_vram)", children: [
              "vram ",
              fmtBytes(m.sizeVram)
            ] }),
            /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("span", { title: "When Ollama will evict this model (keep_alive), from /api/ps (expires_at)", children: [
              "keeps ",
              fmtExpiry(m.expiresAt, now)
            ] })
          ] })
        ] }, m.name))
      ]
...
function SlotBody() {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { style: { ...container, opacity: stale ? 0.55 : 1 }, children: [
    /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { style: { display: "flex", alignItems: "center", gap: 8 }, children: [
      backendLabel(snapshot2) !== null && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
        "span",
        {
          title: `engine: ${backendLabel(snapshot2)}`,
          style: { fontSize: 12, color: "color-mix(in srgb, currentColor 55%, transparent)" },
          children: [
            "\xB7 ",
            backendLabel(snapshot2)
          ]
        }
      ),
      ...
      /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)(
        "span",
        {
          ...
          children: [
            ...
            chip.detail !== null ? chip.detail : bareStateWord(chip, snapshot2)
          ]
        }
      )
    ] }),
    ...
    snapshot2.ollama !== null && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(OllamaCard, { ollama: snapshot2.ollama, now }),
